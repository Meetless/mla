...
run_installer() {
  # MLA_VERSION pins the exact release; MLA_NO_WIRE/MLA_NO_MODIFY_PATH (exported by
  # canary_init) keep it from touching Claude Code or the runner's shell rc.
  MLA_VERSION="$VERSION" sh "$CANARY_ROOT/install.sh" \
...
# 3. macOS recovery reproduction: if a user's mla somehow carries quarantine (a
