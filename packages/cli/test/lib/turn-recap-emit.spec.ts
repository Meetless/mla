...
describe("postTurnRecapToIntel", () => {
  ...
  it("pins the recap projection: exactly these keys reach intel", async () => {
    ...
    expect(Object.keys(body.recap).sort()).toEqual([
      ...
      "offered_source_ids",
      "opened_source_ids",
      // G1 (2026-08-11): ships because `engaged_source_ids` ships, and from this date the
      // union has a fourth input. Without it a fleet reader sees ids in `engaged` that
      // appear in neither `referenced` nor `opened` and has no column that explains them.
      "path_targeted_source_ids",
      "pull_count",
      "ran",
