...
// Both maps ARE SCOUT_AGENT_NAME, not a copy of it: on this surface the dispatch name and
// the agent name are the same string BY DEFINITION. Re-listing the roles here was a second
// place a new scout had to be registered, and the only thing a divergence between the two
// copies could ever mean was a bug.
export const LEGACY_SURFACE: SurfaceNaming = {
  scoutDispatch: SCOUT_AGENT_NAME,
  scoutAgentName: SCOUT_AGENT_NAME,
  mlaCommand: "mla",
};
...
export const PLUGIN_SURFACE: SurfaceNaming = {
  scoutDispatch: {
    ...
    reconciliation: "mla:reconciliation-scout",
  },
  scoutAgentName: {
    documentation: "doc-scout",
    history: "history-scout",
    reconciliation: "reconciliation-scout",
  },
...
// --- Roster derivations for the onboard skill body -------------------------------
//
...
// `enrich ingest`); this skill owns only the agent-driven middle: dispatch the
...
// reads `naming.scoutDispatch[role]`, and every count, roster and dispatch line is
// derived from SCOUT_NAMES rather than written out. Every EXECUTABLE `mla` invocation
...
Tell An this is the longest step and that you will go quiet while the scouts read, for example: "This is the part that takes a minute or two. ${scoutCount.charAt(0).toUpperCase()}${scoutCount.slice(1)} scouts are reading your docs and history in parallel; I will be quiet until they finish. Please keep this session open." Say this BEFORE you dispatch, because once the scouts are running you cannot post another update until they all return.
...
Run an accepting form (\`--all\` or \`--only\`) ONLY on An's selection or explicit request. \`enrich accept\` never touches the governed KB; decisions and deprecations are governed knowledge and are reported as skipped, never written to the rule file. What it DOES write is two things: it mints each accepted rule on the rule authority (the \`minted\` array, under \`authorityScope\`) and it materializes the local projection \`.meetless/rules.md\`. mla neither commits nor pushes, so An shares that file with teammates by committing it himself.
If An accepted nothing, say so plainly and stop; there is no Step ${stepProve} without a minted rule.

Step ${stepProve}: Show him the first rule actually reaching the agent (only if Step ${stepAccept} minted at least one).
Onboarding is where a new workspace stops being empty, and it is the only moment where the whole mechanism can be demonstrated end to end on his OWN content in a few seconds. Do not skip it because the accept "succeeded": minting is not delivery, and until he has seen a rule arrive he has taken the product on faith.
  - Minted is not delivered. The accept result carries \`delivered\`: false means the rules are live on the authority but the NEXT turn will not carry them, because this repo's local cache has not been refreshed. When it is false (or absent), run \`${naming.mlaCommand} scan\` first. When it is true, do not run a redundant scan.
  - Run \`${naming.mlaCommand} status\` and show An its two rule lines verbatim. They answer two different questions and you must not merge them: one reports how many governed floor rules are CONFIGURED for injection, the other reports what the last hook turn actually DELIVERED. Right after a scan the delivery line still describes the previous turn, which is correct and worth saying out loud rather than hiding: it becomes current on his next prompt.
  - Then give him one concrete thing to try. Quote the statement of a rule that was just minted, and hand him a short question about THIS repository whose answer that rule governs, so the next thing he types is the demonstration. Derive the question from the rule's own subject; never invent a scenario the repo does not have, and never dress a synthetic example up as something the scouts discovered.
Say what changed, in his terms: from here on that decision arrives in every session automatically, and he never re-explains it.
...
`;
    default:
      return assertNever(role, "renderScoutAgent");
  }
}
