// Loopback-PKCE browser login for first-party NATIVE clients (RFC 8252).
//
// EXTRACTED, NOT WRITTEN. This is `meetless-cli/packages/cli/src/lib/login.ts`
// moved into a package any first-party client can import, so a desktop app or an
// editor plugin does not have to depend on `packages/cli` to sign a human in.
// Proposal 4.2.3 is explicit that the work here is extraction: the CLI's flow is
// already RFC 8252's recommended native-app flow, and the earlier premise that a
// non-CLI client "has no loopback listener it should be binding" was retracted on
// 2026-08-19. Device authorization stays a contingency for a named client that
// provably cannot bind loopback or open a browser; until one exists, building it
// is infrastructure for a hypothetical.
//
// TWO THINGS BECAME PARAMETERS, and both were hard-coded before because there was
// only ever one client:
//
//   clientId   sent as `client_id` on the authorize URL. "mla" is now the CLI's
//              value rather than the module's, so a second client is a different
//              string and not a fork of this file.
//   fetchImpl  the CLI routes every outbound call through its egress wrapper, so
//              the transport is injected rather than imported. A client with no
//              such wrapper passes global fetch.
//
// Security invariants preserved verbatim from the original:
...
/**
 * The outbound transport. The CLI passes its egress wrapper (which enforces the
 * host allowlist and body redaction); a client without one passes global fetch.
 *
 * Deliberately NOT the DOM `fetch` signature: the CLI's wrapper takes a structured
 * body and a destination label, and narrowing this to `RequestInit` would force
 * every caller to re-serialize what their transport already understands.
 */
export type NativeAuthFetch = (
  destination: "control",
  url: string,
  init: {
    method: string;
...
export async function exchangeGrant(
  ...
  try {
    const res = await deps.fetchImpl("control", url, {
      method: "POST",
      ...
      body: { code, codeVerifier, userAgent: deps.userAgent },
...
export interface BrowserLoginOptions {
  controlUrl: string;
  // Which first-party client is asking. Sent as `client_id`; the CLI passes "mla".
  clientId: string;
  // Outbound transport + the UA string recorded on the new Session.
  fetchImpl: NativeAuthFetch;
  userAgent: string;
...
export async function runBrowserLogin(opts: BrowserLoginOptions): Promise<TokenBundle> {
  ...
  const exchangeFn =
    opts.exchangeFn ??
    ((controlUrl: string, code: string, codeVerifier: string) =>
      exchangeGrant(controlUrl, code, codeVerifier, {
...
async function runLoopbackLogin(
  ...
  const authUrl = buildAuthUrl(consoleUrl, { state, challenge, port, clientId: deps.clientId }).toString();
